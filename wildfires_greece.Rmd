...
head(fdreports_clean |>
  dplyr::select(1:4, 7:10, 15:16, 32:34, 38:42))
...
burnedarea <- fdreports_clean |>
  dplyr::select(7, Woods, TotalIncLosses, a_month)

#For the plot based only on fd reports
#suppressWarnings({
burnedarea |>
  ggplot()+
  aes(x = burnedarea$StartDate, y = as.double(TotalIncLosses), color = a_month)+
  geom_point() +
  labs(title = "Forest area lost per day of the year 2021 (by fire brigade reports)",
       x = NULL,
...
Mapping out different airframe use for the 3 summer months
